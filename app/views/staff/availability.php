...
(() => {
    ...
    function bindTimeRange(startId, endId) {
        ...
        function rebuild() {
            ...
            placeholder.value = '';
            placeholder.textContent = start ? '-- Select end time --' : '-- Select start time first --';
            endEl.appendChild(placeholder);

            if (!start || startMins < 0) {
                endEl.disabled = true;
                return;
            }
            ...
            for (const opt of allOptions) {
                ...
                if (toMinutes(opt.value) <= startMins) continue;

                const next = document.createElement('option');
                next.value = opt.value;
                next.textContent = opt.textContent;
                if (opt.value === selected) next.selected = true;
                endEl.appendChild(next);
                allowedCount++;
            }

            endEl.disabled = (allowedCount === 0);
            if (allowedCount === 0) {
    ...
    bindTimeRange('weekly_start', 'weekly_end');
    bindTimeRange('adj_start', 'adj_end');
})();
</script>
